...
public class BinaryTreeView extends View {
    ...
    public void initialize() {
        //Log.e("BSTView :", "visibility = "+(this.getVisibility()==VISIBLE));
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        //Log.e("onMeasure :", "measured height = "+TreeNode.treeHeight);
        setMeasuredDimension(widthMeasureSpec, 30*TREE_SIZE);
    }
                ...
                case MotionEvent.ACTION_DOWN:
                    ...
                    if (hitValue != -1) {
                        ...
                        if (hitValue != targetValue) {
                            if(searchSequence.indexOf(new Integer(hitValue))>searchPosition)
                                searchSequence.remove(new Integer(hitValue));
                            tree.invalidateNode(targetValue);
                        }
                        StringBuffer serachseq=new StringBuffer("");
                        searchPosition++;
                        for(int x: searchSequence)
                            serachseq.append(x).append(" ");
                        Log.e("serach seq :", serachseq.toString());
                        Log.e("next searchPosition :", ""+searchPosition);
                        updateMessage();
                        return true;
                    }
            }
